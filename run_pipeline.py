import os
import sys
import time

# Add project root to path
sys.path.append(os.path.dirname(os.path.abspath(__file__)))

from src.rotomcv.pipeline_manager import CardPipeline

if __name__ == "__main__":
    project_root = os.path.dirname(os.path.abspath(__file__))
    dataset_csv = os.path.join(project_root, "data", "manifests", "dataset copy.csv")
    
    # Use a separate output directory for hybrid results
    output_dir = os.path.join(project_root, "data", "pipeline_output_hybrid")
    
    print("Initializing Pipeline with Hybrid Engine (Paddle + Tesseract)...")
    start_init = time.time()
    # verify hybrid is default but explict is better for testing
    pipeline = CardPipeline(output_dir=output_dir, ocr_engine_type="hybrid") 
    print(f"Initialization took {time.time() - start_init:.2f}s")
    
    print("Running Batch Processing...")
    start_run = time.time()
    pipeline.run_batch(dataset_csv, output_csv_name="results_hybrid.csv")
    end_run = time.time()
    
    print(f"Total Run Time: {end_run - start_run:.2f}s")
